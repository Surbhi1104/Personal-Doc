            ...
            try:
                for i in range(len(r)):
                    ...
                    if(words==["'s"] or words=="'"):
                        ...
                        s=r[i+1]+" "+r[i-1]
                        symptoms.append(s)
                    elif(tagged[0][1]=='NN' or tagged[0][1]=='JJ') and (tagged[0][0] not in ignore_words):
                        if i>0 and i<len(r)-1:
                            if (nltk.pos_tag(nltk.word_tokenize(r[i+1]))[0][1]!='IN') and (nltk.pos_tag(nltk.word_tokenize(r[i-1]))[0][1]!='IN') and (nltk.word_tokenize(r[i-1])!=["'s"]) and (nltk.word_tokenize(r[i+1])!=["'s"]):
                                symptoms.append(tagged[0][0])
                        elif(nltk.word_tokenize(r[i-1])!=["'s"]):
                            symptoms.append(tagged[0][0])
